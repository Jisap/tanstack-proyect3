...
// Este middleware está diseñado para proteger Server Functions (creadas con createSeverFn)
...
export const authMiddleware = createMiddleware({ type: 'request' }).server(
  async ({ next, request }) => {
    ...
    if (                                                    // Si la url no empieza con /dashboard o /api/ai (rutas protegidas)
      ...
      return next()                                         // deja pasar la solicitud sin comprobar nada más
    }                                                       // Pero si la url si empieza con /dashboard o /api/ai hay que comprobar que se esta autenticado.
